from __future__ import annotations

from typing import List, TYPE_CHECKING

from core.heatmap.birdseye.birds_eye_view_heatmap import BirdsEyeViewHeatmap
from core.heatmap.clusters.cluster_heatmap import ClusterHeatmap
from core.heatmap.clusters.cluster_heatmap_builder import ClusterHeatmapBuilder
from core.momentum.domain import TrackUpdate

if TYPE_CHECKING:
    from core.heatmap.birdseye.cluster_birds_eye_view_heatmap_builder import (
        ClusterBirdsEyeViewHeatmapBuilder,
    )


class ClusterBirdsEyeViewHeatmap(BirdsEyeViewHeatmap):
    def __init__(self, builder: "ClusterBirdsEyeViewHeatmapBuilder"):
        super().__init__(builder)

        eps_cells = builder.cluster_eps_meters * self.scale
        inner_builder = (
            ClusterHeatmapBuilder()
            .with_width(self.width)
            .with_height(self.height)
            .with_frames(builder.frames_count)
            .with_fps(builder.fps)
            .with_momentum_buffer_size(builder.momentum_buffer_size)
            .with_max_lost_frames(builder.max_lost_frames)
            .with_cluster_eps(eps_cells)
        )
        if builder.half_life_time is not None:
            inner_builder.with_half_life_time(builder.half_life_time)

        self._inner: ClusterHeatmap = inner_builder.build()

    def handle(self, updates: List[TrackUpdate]):
        self._inner.handle([self._project_update(update) for update in updates])

    def get_heatmap(self):
        return self._inner.get_heatmap()

    def apply_decay(self):
        self._inner.apply_decay()
