...
- [x] `POST /api/videos/{video_id}/calibration` computes a `cv2.getPerspectiveTransform` matrix from 4 index-matched point pairs and persists it per-`video_id`; `404` for unknown video, `400 invalid_calibration_points` for degenerate/collinear points (round-trip-reprojection check, since `cv2.getPerspectiveTransform` doesn't raise on its own for that case). Covered by `tests/integration/test_calibration.py`.
- [x] `POST /api/videos/{video_id}/heatmaps` uses a video's saved calibration matrix if one exists, otherwise falls back to `CameraInfo`'s hardcoded matrix — both paths asserted directly (captured `transformation_matrix` arg to the enqueued job) in `tests/integration/test_calibration.py`.
- [ ] Full browser click-through of upload → calibration screen → mark 4 points → submit → heatmap menu — attempted live (local dev backend on an alternate port, real Chrome tab, real uploaded video) but interrupted mid-debugging a frame-capture issue (the calibration screen's Konva canvas wasn't progressing past "loading frame") before reaching a conclusion. Re-verify before treating the frontend calibration flow as done.
