...
const LINE_POINT_COLOR = '#22c55e';
const INSIDE_POINT_COLOR = '#3b82f6';

interface TripwirePickerProps {
  videoUrl: string;
  initial?: { p1: Point; p2: Point; insidePoint: Point } | null;
  onSubmit: (p1: Point, p2: Point, insidePoint: Point) => void;
  onCancel: () => void;
}

/** Grabs a static frame from the uploaded video and lets the user mark two
 * points forming the tripwire line, then a third point indicating which side
 * of the line is "inside" — all in the same Konva-on-a-captured-frame
 * pattern as PerspectiveCalibrator, rendered as a modal overlay. */
export function TripwirePicker({ videoUrl, initial, onSubmit, onCancel }: TripwirePickerProps) {
  ...
  useEffect(() => {
    ...
    const { p1, p2, insidePoint } = initialRef.current;
    setPoints([p1, p2, insidePoint].map(([x, y]): Point => [x * scale, y * scale]));
  }, [naturalSize]);
  ...
  const handleSubmit = () => {
    ...
    const [p1, p2, insidePoint] = points.map(([x, y]): Point => [x / scale, y / scale]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="flex flex-col gap-4 items-center w-full max-w-[720px] bg-white rounded-lg p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-medium">Wybierz tripwire</h2>
        <p className="text-sm text-gray-500 text-center max-w-md">
          Zaznacz 2 punkty tworzące linię tripwire, a następnie 3. punkt (na niebiesko) wskazujący stronę
          "wewnątrz" linii.
        </p>
        ...
        {frameImage && naturalSize && (
          ...
          >
            <Layer>
              ...
              {points.length > 1 && (
                <Line points={points.slice(0, 2).flat()} stroke={LINE_POINT_COLOR} strokeWidth={2} />
              )}
              {points.map(([x, y], i) => {
                const isInsidePoint = i === 2;
                const color = isInsidePoint ? INSIDE_POINT_COLOR : LINE_POINT_COLOR;
                return (
                  <Fragment key={i}>
                    <Circle
                      x={x}
                    ...
                    <Text
                      x={x + 10}
                      y={y - 20}
                      text={isInsidePoint ? 'Wewnątrz' : String(i + 1)}
                      fill={color}
                      fontSize={16}
                      fontStyle="bold"
                    />
