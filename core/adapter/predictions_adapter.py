    ...
    def to_predictions(self, raw_predictions: list[Results]) -> list[Prediction]:
        ...
        for prediction in raw_predictions:
            ...
            if prediction.boxes.is_track:
                for box_id, box in zip(
                    prediction.boxes.id.tolist(), prediction.boxes.xywh.tolist()
                ):
                    x, y, w, h = box
                    pred.points += [Point(float(x), float(y) + h / 2, box_id)]
            else:
                pred.points += [
                    Point(float(x), float(y) + h / 2)
                    for x, y, w, h in prediction.boxes.xywh.tolist()
                ]
...
class StreamedYoloPredictionsAdapter(StreamedPredictionsAdapter):
    def to_predictions(self, raw_prediction: Results) -> Prediction:
        if (
            raw_prediction.boxes is None
            or raw_prediction.boxes.xywh is None
            or raw_prediction.path is None
        ):
            return Prediction()
        ...
        if raw_prediction.boxes.is_track:
            for box_id, box in zip(
                raw_prediction.boxes.id.tolist(), raw_prediction.boxes.xywh.tolist()
            ):
                x, y, w, h = box
                pred.points += [Point(float(x), float(y) + h / 2, box_id)]
        else:
            pred.points += [
                Point(float(x), float(y) + h / 2)
                for x, y, w, h in raw_prediction.boxes.xywh.tolist()
            ]
